...
let state = {
  ...
  flags: { carol_trust:0 }, muted: false
};
...
function loadGame(){
  ...
  try{ state = JSON.parse(raw); gotoScene(state.sceneId, true); } catch{ newGame(); }
}
function newGame(){
  state = { sceneId: manifest.startScene, x:0,y:0,vx:0,vy:0,facing:1,hp:3,
            inventory:{roomba:0,roomba_mod:0,cracker:0}, flags:{carol_trust:0}, muted:false };
...
async function gotoScene(id, useSaved=false){
  ...
  current.player.w = current.player.sprite.width; current.player.h = current.player.sprite.height;
...
function step(){
  ...
  state.x = clamp((state.x||scene.player.x) + state.vx, current.bounds.left, current.bounds.right - current.player.w);
...
async function loadLevels(){
  try{ return await loadJSON('levels.json'); }
  catch(e){
